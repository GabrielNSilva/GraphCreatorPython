...
def rem_v(v):
	print(grafo[v]['sai'])
	for p in grafo[v]['sai']:
		print(p)
		grafo[p]['entra'].remove(v)
		grafo[v]['sai'].remove(p)
	print(grafo[v]['entra'])
	for p in grafo[v]['entra']:
		print(p)
		grafo[p]['sai'].remove(v)
		grafo[v]['entra'].remove(p)
...
def mostra():
	if len(grafo) == 0:
		print('***Grafo ainda não cadastrado!***')
	else:
...
def ligados(v1, v2):
	if v1 in grafo and v2 in grafo:
		if v2 in grafo[v1]['sai'] or v1 in grafo[v2]['sai']:
			print('Existe aresta entre os vertices informados!')
...
def grau(v):
	if v in grafo:
		print('Grau de saida:\t'+len(grafo[v]['sai']))
		print('Grau de entrada:\t'+len(grafo[v]['entra']))
	else:
		print('***Vertice inexistente!***')
